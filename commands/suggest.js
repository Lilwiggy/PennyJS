...
// GIVE ME WAIFUS
	...
	const filter = (r, u) => (r.emoji.name === '✅' || r.emoji.name === '🗑') && u.id === '232614905533038593';
	if (args.length === 1) {
		...
		if (!args[2]) {
			...
			client.channels.cache.get('452955996629762058').send({embed: {
				...
				const coll = msg.createReactionCollector(filter);
				coll.on('collect', (r) => {
					if (r.emoji.name === '✅') {
...
or, <https://i.imgur.com/uyiWv1h.jpg>`);
		...
		if (!args[2]) {
			...
			message.channel.send('Thank you for your suggestion, I will let you know if your image is accepted!');
			client.channels.cache.get('452969186763866113').send({embed: {
