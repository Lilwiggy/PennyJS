exports.run = function(client, message, args, Discord, connection) {
    ...
    client.checkUser(message.author.id, message.author.avatarURL, function() {
        if (args.length === 1) {
            connection.query("SELECT *,NOW()-INTERVAL 24 HOUR > `CT` AS canGetDaily,(TO_SECONDS(`CT`)-TO_SECONDS(NOW() - INTERVAL 24 HOUR)) AS restTime, NOW()  FROM `User` WHERE `User_ID`=" + message.author.id + "", function(error, results, fields) {
            ...
            if (message.mentions.users.first()) {
                ...
                if (message.mentions.users.first().bot) {
                    ...
                    client.checkUser(message.mentions.users.first().id, message.mentions.users.first().avatarURL, function() {
                        connection.query("SELECT *,NOW()-INTERVAL 24 HOUR > `CT` AS canGetDaily,(TO_SECONDS(`CT`)-TO_SECONDS(NOW() - INTERVAL 24 HOUR)) AS restTime, NOW()  FROM `User` WHERE `User_ID`= " + message.author.id + "", function(error, results, fields) {
