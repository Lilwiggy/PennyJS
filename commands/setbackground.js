...
// Background thingies! (needs more)
	...
	if (args.length === 1) {
		...
		client.setBackground(message.author.id, message.author.displayAvatarURL({ size: 2048, dynamic: true }), 'default', 0, message);
		...
		if (args[1] === 'patreon') {
			connection.query(`SELECT * FROM \`User\` WHERE \`User_ID\` = ${message.author.id}`, (err, res) => {
				...
				if (res[0].patron === 1) {
					client.setBackground(message.author.id, message.author.displayAvatarURL({ size: 2048, dynamic: true }), 'patreon', 0, message);
				} else {
					message.channel.send('This background is for Patreons only.');
				}
			});
		} else {
			client.setBackground(message.author.id, message.author.displayAvatarURL({ size: 2048, dynamic: true }), data[args[1]].name, data[args[1]].price, message);
		}
	} else {
