// You hate penny? Well shame on you but let me know about it.
// (Totally not a rip off from notsobot)
exports.run = (client, message, args, Discord, connection) => {
    if (args.length === 1) {
        message.channel.send('Usage: //complain [thing to complain about]');
    } else {
        if (message.content.substr(client.prefix.length + 9) > 2047) {

        } else {
            let complain = new Discord.RichEmbed()
