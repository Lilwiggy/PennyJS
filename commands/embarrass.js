...
// Mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm yes
	...
	message.channel.createWebhook(message.author.username, { avatar: message.author.displayAvatarURL({ format: 'png', size: 2048, dynamic: true }) }).then((hook) => {
		...
		message.channel.fetchWebhooks().then((hooks) => {
			hooks.first().delete();
			message.channel.createWebhook(message.author.username, message.author.displayAvatarURL({ format: 'png', size: 2048, dynamic: true })).then((hook) => {
				const stuff = require('../modules/embarrass.json');
				const yes = stuff.things[Math.floor(Math.random() * stuff.things.length)];
				hook.send(yes).then(() => hook.delete());
			});
