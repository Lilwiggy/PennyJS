...
pro = require("../modules/profile.js");
...
let command = "profile"
connection.query("SELECT COUNT(*) as `count`, `User_ID` FROM `usercool` WHERE `User_ID` = " + userID + "", function(err, res, fields) {
    if (res[0].count == 0) {
        ...
        connection.query("SELECT *,NOW()-INTERVAL 1 MINUTE > `cool` AS cooldown,(TO_SECONDS(`cool`)-TO_SECONDS(NOW() - INTERVAL 1 MINUTE)) AS restTime, NOW()  FROM `usercool` WHERE `User_ID`='" + userID + "' AND `command` = '" + command + "'", function(err1, res1, fields1) {
