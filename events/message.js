exports.run = (client, message, Discord, connection) => {
	...
	client.checkServer(message.guild.id, message.guild.name, message.guild.iconURL({ format: 'png', size: 2048, dynamic: true }), () => {
		connection.query(`SELECT \`Prefix\`, \`levels\` FROM \`Servers\` WHERE \`ServerID\` = '${message.guild.id}'`, (error, prefix) => {
			...
			client.checkUser(message.author.id, message.author.displayAvatarURL({ format: 'png', size: 2048, dynamic: true }), () => {
						...
						if (res[0].Blacklisted === 1) {
							...
							if (cmd) {
								try {
									...
									client.users.cache.get('232614905533038593').send(`Error:\n${e}\nUsed in:\n${message.content}`);
								}
							} else {
