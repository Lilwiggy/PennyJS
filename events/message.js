...
exports.run = (client, message, Discord, connection) => {
    ...
    connection.query('SELECT * FROM `User` WHERE `User_ID` = ' + message.author.id + '', (err, res, fields) => {
        ...
        try {
            ...
            if (res[0].Blacklisted === 1) { return; }
        } catch (TypeError) {
            console.log('Hi'); // Somehow this works trust me.
        }

        connection.query("SELECT * FROM `Servers` WHERE `ServerID` = '" + message.guild.id + "'", (error, prefix, fields) => {
            if (error) {
                throw new Error(err);
            }
                ...
                if (prefix[0].levels === 1) {
                    connection.query("SELECT * FROM `User` WHERE `User_ID` = '" + message.author.id + "'", (err, res, fields) => {
                        ...
                        connection.query("SELECT *,NOW()-INTERVAL 2 MINUTE > `xp_cool` AS xpAdd FROM `User` WHERE `User_ID` = '" + message.author.id + "'", (err1, res1, fields1) => {
                            ...
                            } else if (res1[0].xpAdd === 1) {
                                let xp = [Math.floor(Math.random() * 50)]; // 50 xp max at random. Just to make leveling up hard as balls
