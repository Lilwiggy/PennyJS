...
// Run some commands on edits
	...
	client.checkServer(nM.guild.id, nM.guild.name, nM.guild.iconURL({ format: 'png', size: 2048, dynamic: true }), () => {
		connection.query(`SELECT * FROM \`Servers\` WHERE \`ServerID\` = '${nM.guild.id}'`, (error, prefix) => {
			client.checkUser(nM.author.id, nM.author.displayAvatarURL({ format: 'png', size: 2048, dynamic: true }), () => {
