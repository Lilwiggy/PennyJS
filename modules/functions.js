module.exports = (client, connection) => {
    ...
    client.checkUser = (userID, UserAvatar, callback) => {
        connection.query("SELECT COUNT(*) AS `count`, `UserAvatar` FROM `User` WHERE `User_ID`='" + userID + "'", (error, results, fields) => {
            if (error) {
                ...
                connection.query("INSERT INTO `User`(`User_ID`) VALUES ('" + userID + "')", (error, results, fields) => {
                    if (error) {
                        throw new Error(error);
                    }
                    callback();
    ...
    client.checkServer = (serverID, serverName, serverIcon, callback) => {
        connection.query("SELECT COUNT(*) AS `count`, `ServerIcon`,`ServerName` FROM `Servers` WHERE `ServerID` ='" + serverID + "'", (error, results, fields) => {
            if (error) {
                throw new Error(error);
            } else if (results[0].count === 0) {
